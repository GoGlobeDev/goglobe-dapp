...
contract GOGAuction is GOGBoardAccessor{
  ...
  event TransToken(address indexed _operator, uint indexed _tokenId);
  ...
  function transToken(uint _tokenId) public whenNotPaused{
    _transToken(_tokenId);
  }

  function batchTransToken(uint[] _tokenIds) public whenNotPaused{
    for (uint i = 0; i < _tokenIds.length; i++) {
      _transToken(_tokenIds[i]);
    }
  ...
  function _transToken(uint _tokenId) private {
    Auction storage auction = tokenToAuction[_tokenId];
    require(now > auction.endTime);
    gogAP.transferFrom(address(this), msg.sender, _tokenId);
    delete tokenToAuction[_tokenId];
    emit TransToken(msg.sender, _tokenId);
  }
}
